...
def graficar_boxplots_respecto_a_columna(
    df,
    columnas_df,
    columna_objetivo,
    nro_columnas=3,
    ...
    for columna in columnas_df:
        ...
        sns.boxplot(
            df, x=columna_objetivo, y=columna, hue=columna_objetivo, ax=ax
        )

        ax.set_title(f"Boxplot {columna} con respecto a {columna_objetivo}")
...
def graficar_series_temporales(
    ...
    nro_filas = int(len(columnas) / nro_columnas)
    remanente = len(columnas) % nro_columnas
    ...
    for columna in columnas:
        ...
        ax.plot(df[columna_x], df[columna])

        ax.set_title(f"Serie temporal {columna}")
